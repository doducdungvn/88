...
Function ReadDictionary()
    ...
    If fso.FileExists(dictPath) Then
        Dim f
        Set f = fso.OpenTextFile(dictPath, 1, False, -1) ' 1 = ForReading, -1 = Unicode/UTF-8 check
        If Err.Number <> 0 Then
            ' Fallback to ASCII
            Err.Clear
            Set f = fso.OpenTextFile(dictPath, 1, False, 0)
        End If
...
Sub WriteDictionary(d)
    Dim f, k
    Set f = fso.OpenTextFile(dictPath, 2, True, -1) ' 2 = ForWriting, True = Create, -1 = Unicode/UTF-8
    If Err.Number <> 0 Then
        Err.Clear
        Set f = fso.OpenTextFile(dictPath, 2, True, 0)
    End If
...
ElseIf action = "getraw" Then
    ...
    If fso.FileExists(dictPath) Then
        Dim fRaw
        Set fRaw = fso.OpenTextFile(dictPath, 1, False, -1)
        If Err.Number <> 0 Then
            Err.Clear
            Set fRaw = fso.OpenTextFile(dictPath, 1, False, 0)
        End If
...
ElseIf action = "saveraw" Then
    ...
    If rawText <> "" Then
        Dim fSaveRaw
        Set fSaveRaw = fso.OpenTextFile(dictPath, 2, True, -1)
        If Err.Number <> 0 Then
            Err.Clear
            Set fSaveRaw = fso.OpenTextFile(dictPath, 2, True, 0)
        End If
